...
def ReceiveVideo(ConnList,ConnSock,ServerAddr,ServerSock,ClientAddr,ClientSock):
	...
	print ConnList,ConnSock
	while 1:
		start = time.time()#用于计算帧率信息
		string_f = ReceiveContent(ClientSock,16)
		string_split = SplitData(string_f,2)
		length = string_split[2]
		#print length
		if length is not None:
			if int(float(length))>0:
				stringData = ReceiveContent(ClientSock, int(float(length)))#根据获得的文件长度，获取图片文件
				#不用处理，直接转发到除了自己外的其他客户端
				try:
					print "Send frames to others!!!"
					ForwardVideo(ConnSock,ClientSock,stringData,string_f)
				except:
					print str(ClientAddr),"Send Error"
				Frame_Num = Frame_Num + 1
				print "Received ",Frame_Num," frame!!!"
			else:#获得图片文件的长度,16代表获取长度
				print str(length)
				ConnList.remove(ClientAddr)
				ConnSock.pop(ClientAddr)
				ClientSock.close()
				break
		else:
			break
